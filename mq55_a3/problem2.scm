...
;;; or
;;; (before ?x (CS100)) to list all courses which have CS100 as a
;;; requirement.
...
;; ----------------- data base written for problems -------------
...
(rule (before ?course-1 ?course-2)
		(or (bef ?course-1 ?course-2)
			 (and (bef ?course-1 ?x)
					(before ?x ?course-2)
					(not (same ?course-1 ?course-2)))))
